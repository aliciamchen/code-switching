...
write_csv(selection_trials_clean, here("data/3pp/earlier-later/selection_trials_clean.csv"))
```
...
ggplot(baseline.trials.social.means, aes(x = `red - blue`, y = empirical_stat)) +
  ...
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), size = 1.5, width = 0.2) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed") +
  labs(x = "red label - blue label", y = "P(correct group)", title = "Study 1b manipulation checks, social goal") +
  scale_y_continuous(limits = c(0, 1)) +
  theme(text = element_text(size = 15), axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
test_oneRef_vs_oneSoc <- function(fit) {
  em <- emmeans(fit, ~ condition | shared) # include shared if interaction matters
  # Collapse across 'shared' by averaging if needed:
  em_collapsed <- summary(em, by = NULL)
  
  ct <- contrast(
    em_collapsed,
    method = list(
      "one refer - one social" = c("either refer"=0, "one refer"=1, "one social"=-1)
    ),
    adjust = "none"  # or "tukey" for familywise error
  )
  
  summary(ct)$p.value < 0.05
}
```

```{r}
mod_ext <- extend(mod, along = "subject_id", n = 90)
```

```{r}
powerSim(mod, test = test_oneRef_vs_oneSoc, nsim = 500)
```
