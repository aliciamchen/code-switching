...
write_csv(data_better_performance, here("data/3pp/free-response/selection_trials_filtered.csv"))
```
...
# save to csv
write_csv(converged_expressions, here("data/3pp/free-response/converged_expressions.csv"))
```

This is where we run `red_social_similarity.py` and the output of that is in `red_social_similarity_results.csv`

```{r}
red_social_similarity <- read_csv(here("analysis/3pp/free-response/red_social_similarity_results.csv"))
```
