...
mod <- glmer(response.unique ~ audience * goal + (1 | subject_id) + (1 | item_id/ option2.tangram), data = diff.tangrams, family = "binomial", control = glmerControl(optimizer = "bobyqa"))
summary(mod)
```
...
```{r}
diff.tangrams$condition <- with(diff.tangrams, interaction(audience, goal, sep = " "))
```

```{r}
mod <- glmer(response.unique ~ condition + (condition | subject_id) + (condition | item_id/ option2.tangram), data = diff.tangrams, family = "binomial", control = glmerControl(optimizer = "bobyqa"))
