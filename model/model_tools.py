...
def get_model_preds(alpha, w_r, w_s, w_c, expt_type):
    ...
    def speaker_func(aud_cond, ttype, alpha, w_r, w_s, w_c, expt_type):
        return lax.cond(
            ...
            lambda: model_3pp_earlier_later.speaker(
                aud_cond, ttype, alpha, w_r, w_s, w_c
            ),
            lambda: model_3pp_shared_unique.speaker(
                aud_cond, ttype, alpha, w_r, w_s, w_c
            ),
        )
    ...
    def single_pred(cond, ttype):
        return lax.cond(
            ...
            lambda _: speaker_func(
                AudienceConditions.EitherGroup, ttype, alpha, w_r, 0, w_c, expt_type
            ),
            lambda _: lax.cond(
            ...
            1, 0
        ]  # probability of choosing the later utterance or the group-specific label
            ...
            for _, row in available.iterrows():
                for condition in Conditions:
                    preds = preds.at[
                        ...
                        :,
                        condition,
                    ].set(model_preds[condition, TangramTypes[row["tangram_type"]]])
            ...
            for _, row in available.iterrows():
                for condition in Conditions:
                    preds = preds.at[
                        ...
                        model_preds[
                            condition, 0
                        ]  # both columns are the same because tangram type here doesn't matter
                    )
...
def fit_params_overall(data_organized, tangram_info, params_list, expt_type):
    ...
    def single_nll(params):
        ...
        model_organized = format_model_preds(
            preds, data_organized, tangram_info, expt_type
        )
...
def fit_params_participant(data_organized, tangram_info, params_list, expt_type):
    ...
    def single_nlls(params):
        ...
        nlls = jax.vmap(lambda i: compute_nll(data_all[..., i], model_all[..., i]))(
            jnp.arange(total_participants)
        )
        return nlls
